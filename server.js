...
//* Lets now serve our frontend
app.use(express.static(path.join(__dirname, "frontend/dist"))); // absolute path to our frontend: currenct directory/frontend/dist
...
if (process.env.NODE_ENV === "development") {
  ...
  filename:    (req, file, callback) => {callback(null, file.originalname.slice(0,3) + Date.now().toString().slice(-5));},
});
} else {
  storage = multer.diskStorage({
  destination: (req, file, callback) => {callback(null, "frontend/dist")},
  filename:    (req, file, callback) => {callback(null, file.originalname.slice(0,3) + Date.now().toString().slice(-5));},
});
}
...
app.patch("/updat/:id", upload.single("jacket"), async (req,res,next) => {
  try {
...
app.use((err, req, res, next) => {
  ...
  if (err.code === "LIMIT_FILE_SIZE") {
    return res.stauts(400).json({ error: "File too large. MAx 150kb allowed."})
  }
